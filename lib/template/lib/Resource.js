...
class Resource {
  ...
  /**
   * 收集 datalet 变量
   * @param {Object} obj 散落的数据
   * @return {void}
   */
  addDatalet(obj) {
    this._datalets = Object.assign(this._datalets, obj);
  }
  ...
  require(id) {
    ...
    if (res) {
      //rgy 有打包的加载打包内容
      if(res.pkg && this.manifest.pkg && this.manifest.pkg[res.pkg]){
        var pkg = this.manifest.pkg[res.pkg];
        res = pkg;
        rId = res.uri;
      }
      if (this._loaded.hasOwnProperty(rId)) return true;
   ...
   * @param {Function} renderFn 渲染函数, 用来渲染子模板
   * @return {String} 引入的文件的 HTML
   */
  include(file, locals, renderFn) {
    let id = file;
    let html = '';
    let got = false;
    if (file.indexOf('.') === -1) {
  ...
  // xiaoqing modify 如果uri里面已经包含绝对路径了，使用uri里面的绝对路径 2016-9-22 15:34:08
  genComboURI(collect) {
    let combo = '';
    if (collect.length == 1) {
      return this.comboURI(collect[0]);
    }
    //获取公共路径
    collect.forEach((uri) => {
      var arr = uri.split('//')[1].split('/');
      arr.shift();
      combo += ',' + arr.join('/');
    });
    const url = this.getComboPattern();
   ...
   * @returns {*}
   */
  getSameUrlPart(url1, url2) {
    if (!url1) {
      return "";
    }
    //先判断是否为全路径 http://xx.com  https://xx.com  //xx.com
    var reg = /^((?!https?\:)?\/\/)(.*)/;
    if (!reg.test(url1)) {
      return "";
    }
    ...
    var path = RegExp.$2;
    var p = path.split('/');
    while (p.length && url2.indexOf(p.join('/')) < 0) {
      p.pop();
    }
  ...
  getComboPattern(domain) {
    return (domain || this.domain) + (this.manifest.comboPattern || '/co??%s');
  }
  ...
  renderCss() {
    // console.log('this._collect', this._collect);
    ...
    if (this._collect.css && this._collect.css.length) {
      this._collect.css.forEach(this.manifest.combo ? function(uri) {
        ...
        if (cache.hasOwnProperty(uri)) {
          ...
          let path = Resource.root + '/' + uri.replace('/public', '');
          try {
  ...
  render(out) {
    if (this._usePagelet) {
      ...
      if (this._useATF) {
        out = replace(out, this.CSS_HOOK, this.renderATFCss());
        out = replace(out, this.ATF_HOOK, this.renderBTFCss());
      } else {
        out = replace(out, this.CSS_HOOK, this.renderCss());
      }
      out = replace(out, this.JS_HOOK, this.renderJs(), true);
    }
    this.destroy();
    return out;
  }
 ...
 * JS 占位符
 */
Resource.JS_HOOK = Resource.prototype.JS_HOOK = '<!--PAGELET_JS_HOOK-->';

/**
 * CSS 占位符
 */
Resource.CSS_HOOK = Resource.prototype.CSS_HOOK = '<!--PAGELET_CSS_HOOK-->';

/**
 * 首屏占位符
 */
Resource.ATF_HOOK = Resource.prototype.ATF_HOOK = '<!--PAGELET_ATF_HOOK-->';
...
Resource.configure = function configure(opt) {
  ...
  const root = opt.root;
  assert.notEqual(manifest, undefined, '必须提供 manifest 参数来提供`资源映射表`');
  assert.notEqual(root, undefined, '必须提供 root 参数, 作为静态文件的根目录');

  logger = opt.logger || console;

  Resource.root = opt.root;

  // 支持 manifest 的不同读取方式
  let manifestFn;
  if (typeof opt.manifest === 'function') {
    manifestFn = opt.manifest;
  } else if (typeof opt.manifest === 'object') {
    manifestFn = function() {
      return opt.manifest;
    };
  } else {
    manifestFn = function() {
      return JSON.parse(fs.readFileSync(opt.manifest, 'utf8'));
    };
  }
  ...
  Object.defineProperty(Resource, 'manifest', {
    configurable: true,
    get: function() {
      if (!cache || !manifestCache) {
        manifestCache = manifestFn();
      }
